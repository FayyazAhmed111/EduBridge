...
export const deleteUser = async (req, res) => {
  try {
    ...
    await auditLog(req, {
      ...
      targetType: "user",
      targetId: user._id,
      message: `User soft deleted by admin ${req.user.email}`,
      severity: "medium",
    });
  } catch (e) {
...
export const restoreUser = async (req, res) => {
  try {
    ...
    await auditLog(req, {
      ...
      targetType: "user",
      targetId: user._id,
      message: `User restored by admin ${req.user.email}`,
      severity: "low",
    });
  } catch (e) {
...
// Suspend user (with OTP if admin)
export const suspendUser = async (req, res) => {
  try {
    ...
    if (reason) html += `<p><b>Reason:</b> ${reason}</p>`;
...
// Unsuspend user (with OTP if admin)
export const unsuspendUser = async (req, res) => {
  try {
    ...
    await sendMail({
      ...
      html: `<p>Hello ${user.name},</p><p>Your account has been reactivated by an administrator. You can now log back in.</p>`
    });
